...
def file_upload_to_cloud(doc, method=None):
    ...
    if not backend:
        return
    ignore_doctypes = frappe.local.conf.get("ignore_multi_cloud_storage_doctype") or [
        "Data Import"
    ]
    ...
    if doc.is_private:
        ...
        file_url = (
            backend.get_public_url(key) if hasattr(backend, "get_public_url") else path
        )
    try:
        os.remove(file_path)
    except OSError:
...
def migrate_existing_files():
    ...
    return {
        "migrated": migrated,
        "total": len(files),
        "skipped": skipped_no_url_or_cloud
        + skipped_not_local
